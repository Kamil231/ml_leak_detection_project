...
def display_LightGBM(confusion_matrix_best_nodes_df_LGB, confusion_matrix_df_LGB, wn):
    with st.expander("Optymalizacja LightGBM - Wyniki"):
        ...
        with st.expander("Model LightGBM dla wszystkich węzłów"):

            confusion_matrix_df_LGB['TPR'] = confusion_matrix_df_LGB['TP'] / (confusion_matrix_df_LGB['TP'] + confusion_matrix_df_LGB['FN'] + 1e-9)          
            confusion_matrix_df_LGB['FPR'] = confusion_matrix_df_LGB['FP'] / (confusion_matrix_df_LGB['FP'] + confusion_matrix_df_LGB['TN'] + 1e-9)         
            confusion_matrix_df_LGB['Precision'] = confusion_matrix_df_LGB['TP'] / (confusion_matrix_df_LGB['TP'] + confusion_matrix_df_LGB['FP'] + 1e-9)    

            confusion_matrix_df_LGB['F1'] = 2 * (confusion_matrix_df_LGB['Precision'] * confusion_matrix_df_LGB['TPR']) / (confusion_matrix_df_LGB['Precision'] + confusion_matrix_df_LGB['TPR'] + 1e-9)

            st.set_page_config(layout="wide")

            unique_leaks = sorted(
                confusion_matrix_df_LGB['leak_diameter_parameter'].astype(str).unique(),
            ...
            with st.expander("Krzywa ROC"):
                ...
                for leak in unique_leaks:
                    group = confusion_matrix_df_LGB[confusion_matrix_df_LGB['leak_diameter_parameter'].astype(str) == leak]
            ...
            with st.expander("Macierz Pomyłek (Confusion Matrix)"): 
                    ...
                    with col_params_cm:
                        ...
                        selected_leak_cm = st.selectbox(
                            ...
                            key="LightGBM_cm_leak"
                        )
                        
                        unique_thp_cm = sorted(confusion_matrix_df_LGB['decision_threshold'].dropna().unique())
                    ...
                    filtered_cm_data = confusion_matrix_df_LGB[
                        (confusion_matrix_df_LGB['leak_diameter_parameter'].astype(str) == selected_leak_cm) &
                        (confusion_matrix_df_LGB['decision_threshold'] == selected_thp_cm)
                    ]
        ...
        with st.expander("LightGBM: wybor najlepszych wezlow i thresholdow"):

            top_nodes_path = SIMULATION_CONFIG.output_folder / 'pickle' / 'top_nodes_lgb.pkl'
            ...
            confusion_matrix_best_nodes_df_LGB_local = confusion_matrix_best_nodes_df_LGB.loc[confusion_matrix_best_nodes_df_LGB['budget'] == budget]

            confusion_matrix_best_nodes_df_LGB_local['TPR'] = confusion_matrix_best_nodes_df_LGB_local['TP'] / (confusion_matrix_best_nodes_df_LGB_local['TP'] + confusion_matrix_best_nodes_df_LGB_local['FN'] + 1e-9)          
            confusion_matrix_best_nodes_df_LGB_local['FPR'] = confusion_matrix_best_nodes_df_LGB_local['FP'] / (confusion_matrix_best_nodes_df_LGB_local['FP'] + confusion_matrix_best_nodes_df_LGB_local['TN'] + 1e-9)         
            confusion_matrix_best_nodes_df_LGB_local['Precision'] = confusion_matrix_best_nodes_df_LGB_local['TP'] / (confusion_matrix_best_nodes_df_LGB_local['TP'] + confusion_matrix_best_nodes_df_LGB_local['FP'] + 1e-9)    

            confusion_matrix_best_nodes_df_LGB_local['F1'] = 2 * (confusion_matrix_best_nodes_df_LGB_local['Precision'] * confusion_matrix_best_nodes_df_LGB_local['TPR']) / (confusion_matrix_best_nodes_df_LGB_local['Precision'] + confusion_matrix_best_nodes_df_LGB_local['TPR'] + 1e-9)

            unique_leaks = sorted(
                confusion_matrix_best_nodes_df_LGB_local['leak_diameter_parameter'].astype(str).unique(),
                key=lambda x: float(x) if x.replace('.', '', 1).isdigit() else float('inf')
            )
            ...
            with st.expander("Tabea wynikow"):
                ...
                selected_leak_dia = st.selectbox("Wybierz Średnicę Wycieku", options=unique_leaks_dia, key='LightGBM')
            ...
            with st.expander("Krzywa ROC"):
                ...
                for leak in unique_leaks:
                    group = confusion_matrix_best_nodes_df_LGB_local[confusion_matrix_best_nodes_df_LGB_local['leak_diameter_parameter'].astype(str) == leak]
                    group_sorted = group.sort_values(by='decision_threshold', ascending=True)
                ...
                # st.plotly_chart(fig_roc, use_container_width=True)
                st.plotly_chart(fig_roc, use_container_width=True, key=f"roc_chart_LightGBM")
            ...
            with st.expander("Macierz Pomyłek (Confusion Matrix)"): 
                    ...
                    with col_params_cm:
                        ...
                        selected_leak_cm = st.selectbox(
                            "Wybierz Analizowany Wyciek", 
                            options=unique_leaks_cm, 
                            key="LightGBM_cm_leak_bn"
                        )
                        
                        unique_thp_cm = sorted(confusion_matrix_best_nodes_df_LGB_local['decision_threshold'].dropna().unique())
                        selected_thp_cm = st.selectbox(
                            "Wybierz Threshold (THP)", 
                            options=unique_thp_cm, 
                            index=len(unique_thp_cm)//2, 
                            format_func=lambda x: f"{x:.2f}", 
                            key="LightGBM_cm_thp_bn"
                        )

                    filtered_cm_data = confusion_matrix_best_nodes_df_LGB_local[
                        (confusion_matrix_best_nodes_df_LGB_local['leak_diameter_parameter'].astype(str) == selected_leak_cm) &
                        (confusion_matrix_best_nodes_df_LGB_local['decision_threshold'] == selected_thp_cm)
                    ]
                    ...
                    with col_plots_cm:
                        if filtered_cm_data.empty:
                            ...
                            st.plotly_chart(fig_cm, use_container_width=True, key="unique_cm_LightGBM")
