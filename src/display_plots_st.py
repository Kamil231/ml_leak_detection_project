...
with open(pickle_path + 'confusion_matrix_best_nodes_df.pkl', 'rb') as file:
    confusion_matrix_best_nodes_df_XGB = pickle.load(file)
...
with st.expander("Optymalizacja XGBoost - Wyniki", expanded=True):
    ...
    with st.expander("XGBoost: wybor najlepszych wezlow i thresholdow", expanded=True):
        ...
        confusion_matrix_best_nodes_df_XGB = confusion_matrix_best_nodes_df_XGB.loc[confusion_matrix_best_nodes_df_XGB['budget'] == budget]

        confusion_matrix_best_nodes_df_XGB['TPR'] = confusion_matrix_best_nodes_df_XGB['TP'] / (confusion_matrix_best_nodes_df_XGB['TP'] + confusion_matrix_best_nodes_df_XGB['FN'] + 1e-9)          
        confusion_matrix_best_nodes_df_XGB['FPR'] = confusion_matrix_best_nodes_df_XGB['FP'] / (confusion_matrix_best_nodes_df_XGB['FP'] + confusion_matrix_best_nodes_df_XGB['TN'] + 1e-9)         
        confusion_matrix_best_nodes_df_XGB['Precision'] = confusion_matrix_best_nodes_df_XGB['TP'] / (confusion_matrix_best_nodes_df_XGB['TP'] + confusion_matrix_best_nodes_df_XGB['FP'] + 1e-9)    

        confusion_matrix_best_nodes_df_XGB['F1'] = 2 * (confusion_matrix_best_nodes_df_XGB['Precision'] * confusion_matrix_best_nodes_df_XGB['TPR']) / (confusion_matrix_best_nodes_df_XGB['Precision'] + confusion_matrix_best_nodes_df_XGB['TPR'] + 1e-9)

        unique_leaks = sorted(
            confusion_matrix_best_nodes_df_XGB['leak_diameter_parameter'].astype(str).unique(),
            key=lambda x: float(x) if x.replace('.', '', 1).isdigit() else float('inf')
        )

        with st.expander("Tabea wynikow", expanded=True):
        ...
        with st.expander("Krzywa ROC", expanded=True):
            ...
            for leak in unique_leaks:
                group = confusion_matrix_best_nodes_df_XGB[confusion_matrix_best_nodes_df_XGB['leak_diameter_parameter'].astype(str) == leak]
                group_sorted = group.sort_values(by='decision_threshold', ascending=True)
            ...
            # st.plotly_chart(fig_roc, use_container_width=True)
            st.plotly_chart(fig_roc, use_container_width=True, key=f"roc_chart_{i}")
        ...
        with st.expander("Macierz Pomyłek (Confusion Matrix)", expanded=True): 
                ...
                with col_params_cm:
                    ...
                    unique_thp_cm = sorted(confusion_matrix_best_nodes_df_XGB['decision_threshold'].dropna().unique())
                    selected_thp_cm = st.selectbox(
                        "Wybierz Threshold (THP)", 
                        options=unique_thp_cm, 
                        index=len(unique_thp_cm)//2, 
                ...
                filtered_cm_data = confusion_matrix_best_nodes_df_XGB[
                    (confusion_matrix_best_nodes_df_XGB['leak_diameter_parameter'].astype(str) == selected_leak_cm) &
                    (confusion_matrix_best_nodes_df_XGB['decision_threshold'] == selected_thp_cm)
                ]
