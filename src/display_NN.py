...
def display_NN(confusion_matrix_df, confusion_matrix_best_nodes_df, wn):
	...
	with st.expander("Optymalizacja za pomocą sieci neuronowych - Wyniki"):

		with st.expander("Model NN dla wszystkich węzłów"):

		    confusion_matrix_df['TPR'] = confusion_matrix_df['TP'] / (confusion_matrix_df['TP'] + confusion_matrix_df['FN'] + 1e-9)          
		    confusion_matrix_df['FPR'] = confusion_matrix_df['FP'] / (confusion_matrix_df['FP'] + confusion_matrix_df['TN'] + 1e-9)         
		    confusion_matrix_df['Precision'] = confusion_matrix_df['TP'] / (confusion_matrix_df['TP'] + confusion_matrix_df['FP'] + 1e-9)    

		    confusion_matrix_df['F1'] = 2 * (confusion_matrix_df['Precision'] * confusion_matrix_df['TPR']) / (confusion_matrix_df['Precision'] + confusion_matrix_df['TPR'] + 1e-9)

		    st.set_page_config(layout="wide")

		    unique_leaks = sorted(
		        confusion_matrix_df['leak_diameter_parameter'].astype(str).unique(),
		    ...
		    with st.expander("Krzywa ROC"):
		        ...
		        for leak in unique_leaks:
		            group = confusion_matrix_df[confusion_matrix_df['leak_diameter_parameter'].astype(str) == leak]
		        ...
		        st.plotly_chart(fig_roc, use_container_width=True, key="nn_roc")
		    ...
		    with st.expander("Macierz Pomyłek (Confusion Matrix)"): 
		            ...
		            with col_params_cm:
		                ...
		                unique_thp_cm = sorted(confusion_matrix_df['decision_threshold'].dropna().unique())
		            ...
		            filtered_cm_data = confusion_matrix_df[
		                (confusion_matrix_df['leak_diameter_parameter'].astype(str) == selected_leak_cm) &
		                (confusion_matrix_df['decision_threshold'] == selected_thp_cm)
		            ]
		...
		with st.expander("Siec neuronowa: wybor najlepszych wezlow i thresholdow"):

		    top_nodes_path = SIMULATION_CONFIG.output_folder / 'pickle' / 'top_nodes_nn.pkl'

		    with open(top_nodes_path, 'rb') as f:
		        loaded_top_nodes = pickle.load(f)

		    max_budget = confusion_matrix_best_nodes_df['budget'].max()
		    min_budget = confusion_matrix_best_nodes_df['budget'].min()
		    ...
		    confusion_matrix_best_nodes_df_local = confusion_matrix_best_nodes_df.loc[confusion_matrix_best_nodes_df['budget'] == budget]

		    confusion_matrix_best_nodes_df_local['TPR'] = confusion_matrix_best_nodes_df_local['TP'] / (confusion_matrix_best_nodes_df_local['TP'] + confusion_matrix_best_nodes_df_local['FN'] + 1e-9)          
		    confusion_matrix_best_nodes_df_local['FPR'] = confusion_matrix_best_nodes_df_local['FP'] / (confusion_matrix_best_nodes_df_local['FP'] + confusion_matrix_best_nodes_df_local['TN'] + 1e-9)         
		    confusion_matrix_best_nodes_df_local['Precision'] = confusion_matrix_best_nodes_df_local['TP'] / (confusion_matrix_best_nodes_df_local['TP'] + confusion_matrix_best_nodes_df_local['FP'] + 1e-9)    

		    confusion_matrix_best_nodes_df_local['F1'] = 2 * (confusion_matrix_best_nodes_df_local['Precision'] * confusion_matrix_best_nodes_df_local['TPR']) / (confusion_matrix_best_nodes_df_local['Precision'] + confusion_matrix_best_nodes_df_local['TPR'] + 1e-9)

		    unique_leaks = sorted(
		        confusion_matrix_best_nodes_df_local['leak_diameter_parameter'].astype(str).unique(),
		        key=lambda x: float(x) if x.replace('.', '', 1).isdigit() else float('inf')
		    )
		    ...
		    with st.expander("Tabea wynikow"):
		        ...
		        selected_leak_dia = st.selectbox("Wybierz Średnicę Wycieku", options=unique_leaks_dia, key='nn')
		    ...
		    with st.expander("Krzywa ROC"):
		        ...
		        for leak in unique_leaks:
		            group = confusion_matrix_best_nodes_df_local[confusion_matrix_best_nodes_df_local['leak_diameter_parameter'].astype(str) == leak]
		            group_sorted = group.sort_values(by='decision_threshold', ascending=True)
		        ...
		        st.plotly_chart(fig_roc, use_container_width=True, key=f"roc_chart_nn")
		    ...
		    with st.expander("Macierz Pomyłek (Confusion Matrix)"): 
		            ...
		            with col_params_cm:
		                ...
		                unique_thp_cm = sorted(confusion_matrix_best_nodes_df_local['decision_threshold'].dropna().unique())
		                selected_thp_cm = st.selectbox(
		                    "Wybierz Threshold (THP)", 
		                    options=unique_thp_cm, 
		                    index=len(unique_thp_cm)//2, 
		            ...
		            filtered_cm_data = confusion_matrix_best_nodes_df_local[
		                (confusion_matrix_best_nodes_df_local['leak_diameter_parameter'].astype(str) == selected_leak_cm) &
		                (confusion_matrix_best_nodes_df_local['decision_threshold'] == selected_thp_cm)
		            ]
		            ...
		            with col_plots_cm:
		                if filtered_cm_data.empty:
		                    ...
		                    st.plotly_chart(fig_cm, use_container_width=True, key="unique_cm_nn")
